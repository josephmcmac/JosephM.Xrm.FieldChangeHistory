...
FieldChangePageUtility = function () {
    var that = this;
    ...
    ********************************/
    ...
    this.SetFieldDisabled = function (fieldName, isDisabled, ignoreReadOnly) {
        if (isDisabled == null)
            isDisabled = true;
        var isDirty = Xrm.Page.getAttribute(fieldName).getIsDirty();
        //only change the editability of fields if the user has permissions on it
        if ((that.GetFormType() == that.FormMode.Create && Xrm.Page.getAttribute(fieldName).getUserPrivilege().canCreate)
            || (that.GetFormType() == that.FormMode.Update && Xrm.Page.getAttribute(fieldName).getUserPrivilege().canUpdate)
            || (ignoreReadOnly == true && that.GetFormType() == that.FormMode.ReadOnly && Xrm.Page.getAttribute(fieldName).getUserPrivilege().canUpdate))
    ...
    this.SetFieldValue = function (fieldName, value, dontSubmit) {
...
fieldChangePageUtility = new FieldChangePageUtility();