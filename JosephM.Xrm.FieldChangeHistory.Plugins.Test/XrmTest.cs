...
namespace JosephM.Xrm.FieldChangeHistory.Plugins.Test
{
    ...
    public abstract class XrmTest
    {
        ...
        private Entity _otherAdmin;

        public Entity OtherAdmin
        {
            get
            {
                if (_otherAdmin == null)
                {
                    var otherAdminQuery = new QueryExpression(Entities.systemuser);
                    otherAdminQuery.Criteria.AddCondition(new ConditionExpression(Fields.systemuser_.systemuserid, ConditionOperator.NotEqual, CurrentUserId));
                    var rolesIntersect = otherAdminQuery.AddLink(Relationships.systemuser_.systemuserroles_association.EntityName, Fields.systemuser_.systemuserid, Fields.systemuser_.systemuserid);
                    var rolesJoin = rolesIntersect.AddLink(Entities.role, Fields.role_.roleid, Fields.role_.roleid);
                    rolesJoin.LinkCriteria.AddCondition(new ConditionExpression(Fields.role_.name, ConditionOperator.Equal, "System Administrator"));
                    _otherAdmin = XrmService.RetrieveFirst(otherAdminQuery);
                    Assert.IsNotNull(_otherAdmin);
                }
                return _otherAdmin;
            }
            set
        ...
        private Entity _testTeam;

        public Entity TestTeam
        {
            get
            {
                if (_testTeam == null)
                {
                    _testTeam = XrmService.GetFirst(Entities.team, Fields.team_.name, "Test Team");
                    if (_testTeam == null)
                    {
                        _testTeam = new Entity(Entities.team);
                        _testTeam.SetField(Fields.team_.name, "Test Team");
                        _testTeam.SetOptionSetField(Fields.team_.teamtype, OptionSets.Team.TeamType.Owner);
                        _testTeam.SetLookupField(Fields.team_.businessunitid, XrmService.GetFirst(Entities.businessunit));
                        _testTeam = CreateAndRetrieve(_testTeam);
                        var systemAdminRole = XrmService.GetFirst(Entities.role, Fields.roletemplate_.name, "System Administrator");
                        Assert.IsNotNull(systemAdminRole);
                        XrmService.Associate(Relationships.team_.teamroles_association.Name, _testTeam.Id, Entities.team, Fields.team_.teamid, Entities.role, Fields.role_.roleid, new[] { systemAdminRole.Id }, true);
                    }
                }
                return _testTeam;
            }
        ...
        private Entity _testAccount2;

        public Entity TestAccount2
        {
            get
            {
                if (_testAccount2 == null)
                {
                    _testAccount2 = XrmService.GetFirst(Entities.account, Fields.account_.name, "Test Account 2");
                    if (_testAccount2 == null)
                    {
                        _testAccount2 = new Entity(Entities.account);
                        _testAccount2.SetField(Fields.account_.name, "Test Account 2");
                        _testAccount2 = CreateAndRetrieve(_testAccount2);
                    }
                }
                return _testAccount2;
            }
            set
            {
                _testAccount2 = value;
