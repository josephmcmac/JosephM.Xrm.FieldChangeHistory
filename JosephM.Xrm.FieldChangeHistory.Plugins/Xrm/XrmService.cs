...
namespace JosephM.Xrm.FieldChangeHistory.Plugins.Xrm
{
    public class XrmService : IOrganizationService
    {
        ...
        public string GetFieldAsDisplayString(string recordType, string fieldName, object value, LocalisationService localisationService, bool isHtml = false, string func = null)
        {
            ...
            {
                return GetOptionLabel(osv.Value, fieldName, recordType);
            }
            else if (value is Money m)
            {
                return m.Value.ToString("$##,###,###,###,##0.00");
            }
        ...
        public string GetFieldAsInternalString(string fieldName, string entityType, object value)
        {
            if (value is DateTime dt)
            {
                return dt.ToString("yyyy-MM-dd mm:ss:mm");
            }
            else
            {
                var converted = ConvertToQueryValue(fieldName, entityType, value);
                if (converted is decimal d)
                {
                    return d.ToString("#############0.##########");
                }
                else if (converted is double db)
                {
                    return db.ToString("#############0.##########");
                }
                else
