        ...
        private void UpdateFieldChangeHistory()
        {
            if (IsMessage(PluginMessage.Create, PluginMessage.Update, PluginMessage.Delete) && IsStage(PluginStage.PostEvent) && IsMode(PluginMode.Synchronous))
            {
                ...
                if (FieldChanging(field))
                {
                    ...
                    var createNew = !IsMessage(PluginMessage.Delete);

                    var closeOld = !IsMessage(PluginMessage.Create);

                    if (closeOld)
                    {
                        var oldAssignedId = GetLookupGuidPreImage(Fields.incident_.ownerid);
                        if (oldAssignedId.HasValue)
                        {
